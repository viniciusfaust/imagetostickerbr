    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                if (ids.length <= 10) {
                    client.sendText(id, 'Desculpe, o maximo de grupos suportados e 10. Tchau~').then(() => client.leaveGroup(id))
                } else {
                    client.sendText(id, `Olá *${name}*, obrigado por adcicionar este bot, para ver os comandos digite *#menu*`)
                }
            }))
