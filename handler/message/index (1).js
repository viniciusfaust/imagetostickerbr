    ...
    try {
        ...
        case 'help':
            await client.sendText(from, menuId.textMenu(pushname))
                .then(() => ((isGroupMsg) && (isGroupAdmins)) ? client.sendText(from, 'Menu do admin: *#menuadmin*') : null)
            break
        case 'menuadmin':
            if (!isGroupMsg) return client.reply(from, 'Desculpe, este comando só pode ser usado em grupos! [Group Only]', id)
            if (!isGroupAdmins) return client.reply(from, 'Falhado, este comando só pode ser usado por administradores de grupo! [Admin Group Only]', id)
        ...
        case 'stiker': {
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                client.sendImageAsSticker(from, imageBase64).then(() => {
                    client.reply(from, 'aqui esta seu sticker')
                ...
                */
                ...
                client.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? client.sendText(from, 'Desculpe, o link que enviou não carregou a imagem.. [No Image]')
                    : client.reply(from, 'Aqui está seu sticker')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
                await client.reply(from, 'Sem fotos! Para abrir a lista de comando enviar #menu [Wrong Format]', id)
            }
            break
        }
        case 'stikergif':
        ...
        case 'gifstiker':
        case 'gifsticker': {
            if (args.length !== 1) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            ...
            if (isGiphy) {
                ...
                await client.reply(from, 'desculpe, por enquanto adesivos gif só pode usar links de giphy.  [Giphy Only]', id)
            }
            break
        }
        // Video Downloader
        case 'tiktok':
            if (args.length !== 1) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            if (!isUrl(url) && !url.includes('tiktok.com')) return client.reply(from, 'Desculpe, o link que enviou é inválido.. [Invalid Link]', id)
            ...
            }).catch(() => client.reply(from, 'Falha ao recuperar metadados, link inválido que você enviou. [Invalid Link]', id))
            break
        case 'ig':
        case 'instagram':
            if (args.length !== 1) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            if (!isUrl(url) && !url.includes('instagram.com')) return client.reply(from, 'Desculpe, o link que enviou é inválido.. [Invalid Link]', id)
            await client.reply(from, `_Scraping Metadata..._ \n\n${menuId.textDonasi()}`, id)
            downloader.insta(url).then(async (data) => {
                ...
                .catch((err) => {
                    if (err === 'Not a video') { return client.reply(from, 'Erro, nenhum vídeo no link que você enviou. [Invalid Link]', id) }
        ...
        case 'facebook':
            if (args.length !== 1) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            if (!isUrl(url) && !url.includes('facebook.com')) return client.reply(from, 'Desculpe, a url que você enviou é inválida. [Invalid Link]', id)
        ...
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                await client.reply(from, 'Sem fotos! Para abrir uma maneira de enviar #menu [Wrong Format]', id)
            }
            break
        case 'resi':
            if (args.length !== 2) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
        ...
        case 'translate':
            if (args.length != 1) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            if (!quotedMsg) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            ...
            break
        case 'ceklokasi':
            if (quotedMsg.type !== 'location') return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            console.log(`Request Status Zona Penyebaran Covid-19 (${quotedMsg.lat}, ${quotedMsg.lng}).`)
            const zoneStatus = await getLocationData(quotedMsg.lat, quotedMsg.lng)
            if (zoneStatus.kode !== 200) client.sendText(from, 'somente o strang pode usar este comando.')
            let data = ''
        ...
        case 'kick':
            if (!isGroupMsg) return client.reply(from, 'Desculpe, este comando só pode ser usado em grupos! [Group Only]', id)
            if (!isGroupAdmins) return client.reply(from, 'Falhado, este comando só pode ser usado por administradores de grupo! [Admin Group Only]', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Falhou, por favor adicione bot como administrador de grupo! [Bot Not Admin]', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            if (mentionedJidList[0] === botNumber) return await client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format]', id)
            await client.sendTextWithMentions(from, `Pedido recebido, emitido:\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await client.sendText(from, 'Falhou, você não pode ejetar administração de grupo.')
                await client.removeParticipant(groupId, mentionedJidList[i])
            }
        case 'promote':
            if (!isGroupMsg) return await client.reply(from, 'Desculpe, este comando só pode ser usado em grupos! [Group Only]', id)
            if (!isGroupAdmins) return await client.reply(from, 'Falhado, este comando só pode ser usado por administradores de grupo! [Admin Group Only]', id)
            if (!isBotGroupAdmins) return await client.reply(from, 'Falhou, por favor adicione bot como administrador de grupo! [Bot not Admin]', id)
            if (mentionedJidList.length != 1) return client.reply(from, 'Desculpe, formato de mensagem errado, por favor, verifique o menu. [Wrong Format, Only 1 user]', id)
       ...
       case 'del':
        ...
        case 'everyone':
            ...
            * Check premium feature at https://trakteer.id/red-emperor/showcase or chat Author for Information.
            */
            client.reply(from, 'somente o strang pode usar este comando', id)
            break
        case 'botstat': {
